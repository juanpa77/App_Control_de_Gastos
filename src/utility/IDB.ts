...
export class Idb {
  ...
  async openDB() {
    this.db = await openDB("Month", 1, {
      upgrade(db) {
        ...
        for (let i = 1; i < 13; i++) {
          if (i < 10) {
            const monthStore = db.createObjectStore(`0${i.toString()}`, {
              keyPath: "id",
            });
            monthStore.createIndex("date", "date");
          } else {
            const monthStore = db.createObjectStore(i.toString(), {
              keyPath: "id",
            });
            monthStore.createIndex("date", "date");
          }
  ...
  async getFixedExpenses(store: string) {
    ...
    while (cursor) {
      ...
      const amount: any = this.isCredit(objetTransaction)
        ? objetTransaction.amount
        : 0;
  ...
  async cursor(store: string, rangeDate: string, type: string) {
    ...
    while (cursor) {
      ...
      if (type === this.filterTypeTransaction(objetTransaction)) {
        let amount: any = this.filterTransactionObj(
          rangeDate,
          objetTransaction
        );
  ...
  filterTypeTransaction = (transaction: TransactionData) =>
    transaction.type === "Expenses" ? "Expenses" : "Income";

  isCredit(transaction: TransactionData) {
    if (transaction.category === "credit") return true;
  }

  filterTransactionObj(
    dataRange: string,
    transaction: TransactionData
  ): number {
    let filterDate: number | number[] = this.calcDate(dataRange, new Date());
    const transactionDate = this.calcDate(
      dataRange,
  ...
  calcDate(rangeDate: string, date: Date, type?: string) {
    ...
    if (rangeDate === "Semanal")
      return type === "transactionDate" ? date.getDate() + 1 : date.getDate();
    if (rangeDate === "Diario")
      return type === "transactionDate" ? date.getDate() + 1 : date.getDate();
    return 0;
  }
  ...
  filterWeek() {
    ...
    const date = new Date();
    const calcDayOneTheWeek =
      date.getDate() - date.getDay() + selectedDayOneTheWeek;
    const arrayDate = [];
    for (let i = 0; i < 7; i++) {
