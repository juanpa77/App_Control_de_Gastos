...
export const SelectTransactionType = ()=> {
    ...
    return (
        <div className="select__radio">
            <div className={`icome__radioButton ${toggleClass ? 'active__radioButton': ''}`}
            onClick={(e)=>classTriggerToggle(e)}
            >Ingreso</div>
            <div onClick={(e)=>classTriggerToggle(e)}
            className= {`expense__radioButton ${toggleClass ? '': 'active__radioButton'}`}>
                Gasto
            </div>
        </div>
    )
